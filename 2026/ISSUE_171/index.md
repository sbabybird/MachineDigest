...
[WaylandCraft](https://modrinth.com/mod/waylandcraft)（又名 WaylandMC），一个在 Minecraft 内实现的 Wayland 合成器 Fabric 模组。它可以在 Minecraft 的方块世界中渲染真实的 Wayland 客户端窗口（比如 weston-terminal），并将键盘和鼠标事件转发回去。
